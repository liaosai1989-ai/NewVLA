...
- 执行约束：
  ...
  - `dify_target_key` 与 `dataset_id` 必须以 `task_context.json` 中的显式注入值为准
...
- 最终结果需要上传到 `task_context.json` 中指定的 Dify 目标。
- 调用上传工具时，必须显式传入 `dify_target_key` 和 `dataset_id`。
- 不要自行猜测 Dify 实例或替换目标知识库。
...
- folder 分流：`folder_token` 能命中 `qa_rule_file`、`dify_target_key` 与 `dataset_id`
...
- Dify 目标注入：`task_context.json` 正确写入 `dify_target_key` 与 `dataset_id`，且任务单明确要求按该目标上传
...
- `folder_token` 只做业务分流，映射到 `qa_rule_file`、`dify_target_key` 与 `dataset_id`
- `AGENTS.md` 不维护 `folder_token -> qa_rule_file / dify_target_key / dataset_id` 映射；该映射只存在于运行时配置与 `task_context.json`
- Agent 不负责从仓库文档、`AGENTS.md` 或规则目录中推断 Dify 目标；`dify_target_key` 与 `dataset_id` 必须来自 `task_context.json` 的显式注入
- 配置映射优先于运行时猜测
- 单文档尽量不并发，多文档可并发
- LLM 主输入必须是任务单，而不是机器参数堆砌
- `task_context.json` 必须由任务单显式要求读取，不假设存在隐式自动注入
- `task_prompt.md` 必须要求 Agent 先读 `task_context.json`，再读指定 QA 规则文件，最后按对应 `dify_target_key` 与 `dataset_id` 上传
