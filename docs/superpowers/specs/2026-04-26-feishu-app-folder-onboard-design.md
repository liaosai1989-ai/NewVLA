...
> **落地状态：实现已合入（`onboard/`）；§7.3/清单验收未闭环**（2026-04-28；与文首修订说明中「现网入轨」口径一致。）

## 修订说明

- **2026-04-29（事件驱动 per-doc subscribe；废止「禁止文档级 subscribe」执行口径）**：实机验证「仅夹级 folder subscribe + 开发者后台事件」不足以稳定收 **`drive.file.edit_v1`**。现合同：在收到 **`drive.file.created_in_folder_v1`** 时，由 **`webhook`/`webhook_cursor_executor.worker`** 以应用 **tenant** 对该 **`file_token`** 再调用 [订阅云文档事件](https://open.feishu.cn/document/server-docs/docs/drive-v1/event/subscribe)（query 仅 **`file_type`**=`docx`/`sheet`/…，**不传** `event_type`）。实现模块 `webhook/.../feishu_drive_subscribe.py`。仍 **禁止** 对夹内历史对象 **全量扫目录** 做批量 subscribe；**不**采用用户 OAuth。`feishu-onboard` 仍 **只**做夹级 folder subscribe。`feishu_fetch` 仍 **不**调用 drive subscribe。曾规定「仅夹级、禁止一切 per-doc subscribe」之口径 **已废止**，以本条为准。
...
- **2026-04-27（夹级事件 subscribe；`BugList.md` BUG-006）**：在取得本 route 的 `folder_token` 后、**为分享委托人加文件夹协作者之前**，`onboard` 须以应用身份再调 **`POST https://open.feishu.cn/open-apis/drive/v1/files/{folder_token}/subscribe`**，query 为 **`file_type=folder`**、**`event_type=file.created_in_folder_v1`**（与飞书[订阅云文档事件](https://open.feishu.cn/document/server-docs/docs/drive-v1/event/subscribe) 及仓库内 `webhook/scripts/subscribe_byvwf_tds.py` 一致）。该步是云空间内「夹内新建文件」等事件链的**夹级**前提，与同应用下其它已含 subscribe 的入轨产品对齐；**不在**入轨中枚举夹内已有 docx；**新建**后的 per-doc subscribe 由 **webhook 事件驱动**（详见 **2026-04-29** 首条修订），**禁止**枚举夹内历史全量 subscribe。续跑（已有 `FEISHU_FOLDER_*_TOKEN` 且非 `--force-new-folder`）时仍会**再调一次**该接口（以现网实现为准，是否幂等完全依赖飞书侧返回）。**`verify-delegate` 子命令**仅联调建夹+协作者，**不含**本步。若正文 §6.2「创建文件夹」1–3 步与 §6.3「推荐顺序」未显式写 subscribe，以本段与现网 `FeishuOnboardClient.subscribe_folder_file_created` / `flow.run_onboard` 为准。
