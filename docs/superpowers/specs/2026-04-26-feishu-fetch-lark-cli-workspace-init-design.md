...
- **2026-04-30（BUG-001 / `lark-cli` 仅 PATH、废键 `LARK_CLI_COMMAND`）**：实现真源：`facade` 固定命令名 **`lark-cli`** + **`shutil.which`**；根 `.env` **禁止** **`LARK_CLI_COMMAND`**（`load_feishu_fetch_settings` 见键即失败）。正文凡写「`LARK_CLI_COMMAND` 统一生效」「核对 `LARK_CLI_COMMAND`」「`LARK_CLI_COMMAND` 可执行/找不到」等，**现均指**：固定名 **`lark-cli`** 经 PATH 解析与预检；**非**可配置 CLI 路径环境变量。历史 §2「在所有抓取分支统一生效」语义保留为**消除半接线之设计动机**；**落地形态**= 固定名 + 标准解析，与 [root-env](./2026-04-26-root-env-and-dify-target-contract-design.md) 文首 2026-04-30 修订一致。
